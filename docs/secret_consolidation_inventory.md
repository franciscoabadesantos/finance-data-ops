...
4. Worker auth used both platform identity and shared bearer token patterns simultaneously.
...
| `WORKER_SHARED_TOKEN` | analysis worker env | Analysis/admin HTTP worker | Optional app-layer bearer token for `analysis_job` requests only. Ticker lifecycle requests use Prefect deployments, not this worker. | Medium; analysis job execution path |
...
4. For analysis worker deploy:
   - switch GH auth from `GCP_SA_KEY` JSON to WIF.
   - inject runtime secrets via Cloud Run `--set-secrets` from GSM.
5. Rotate leaked keys (mandatory because plaintext values existed in git history).
...
- Updated `finance-data-ops` worker deploy workflow to:
  ...
  - keep optional `WORKER_SHARED_TOKEN` secret for analysis jobs.
...
3. Set repo vars for worker deploy:
   ...
   - optional: `GSM_WORKER_SHARED_TOKEN_SECRET`, `GCP_WORKER_RUNTIME_SERVICE_ACCOUNT`

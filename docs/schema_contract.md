...
Controlled publication is idempotent and batch-scoped. Raw fact caches upsert by their natural cache keys before entity rows are written. Entity master rows upsert by `entity_id`, listing rows upsert by `symbol`, review rows upsert by `review_key`, publication batches upsert by `batch_id`, and the current pointer upserts by `scope_key`. Entity rows include `publication_batch_id`; publication batches include `publication_gate`, `planned_counts`, `actual_counts`, `verification_summary`, and `blocked_reasons`. A failed gate must not write entity rows.

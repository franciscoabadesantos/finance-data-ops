...
Publication readiness is a separate gate from measurement. Measurement emits a complete `heuristic_attach_audit` for every non-direct attach, not a sample, plus a `publication_gate` summary. The audit verifies deterministic support, conservative legal-name normalization, attached group membership, conflicting LEIs/names, country/entity mismatch signals, provenance, and confidence. CJK/APAC names keep distinctive non-Latin tokens during normalization and only strip legal-form suffixes; distinctive-token removal is always review-required, while CJK-to-Latin acronym collapse and short/acronym-only heuristic attaches need bidirectional ISIN evidence before they are machine-safe: the listing's own or matched compatible ISIN must forward-resolve to the attached LEI. Heuristic attaches without bidirectional evidence are routed to review unless an explicit reviewed-safe decision is supplied. Version-controlled curated identity decisions in `data/entity_identity_curated.json` can attach known symbol-to-LEI mappings with `curated_identity` provenance and evidence metadata. Entity publication helpers build immutable batch snapshots of `entity_master`, `entity_listing`, and review rows, and default to blocking writes if the gate is not `ready_machine_safe`. Provisional single-listing candidates are storable as low-confidence evidence with symbol-scoped provisional ids and do not represent confirmed merges. Multi-listing unresolved candidates remain the curation/provider priority. Re-evaluation triggers include new listings, OpenFIGI refreshes, provider ISIN improvements, new GLEIF ISIN-to-LEI mappings, LEI expansion additions, legal-name candidate changes, manual review decisions, and corroborating provider evidence. The relationship atlas consumes the current published `tracked` snapshot to project listing-level evidence onto economic entities; source prices, technicals, scorecards, and relationship edges remain listing-level and are never blended across listings.

Controlled side-by-side publication is cache-first and versioned by publication batch. `scripts/publish_entity_identity_side_by_side.py` dry-runs by default, requires `--refresh-live` for live OpenFIGI/yfinance/GLEIF calls, separates `--apply-caches` from `--apply-entities`, writes source-cache facts before entity rows when both are requested, and blocks entity writes unless the green publication gate, zero unresolved multi-listing entities, zero group conflicts, and zero heuristic review-required rows all hold for the measured scope. `feature_store.entity_identity_publication_batch` stores gate output, planned/actual counts, and verification summaries; `feature_store.entity_identity_publication_current` is the atomic current pointer by scope. Entity and listing rows are immutable within `(publication_batch_id, entity_id)` and `(publication_batch_id, symbol)` respectively. The Feature Store reads one exact pointer/batch, materializes listing-to-atlas-node aliases and snapshot provenance, and the Backend resolves any sibling listing to that materialized economic node.

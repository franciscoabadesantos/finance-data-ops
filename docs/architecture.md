...
Entity identity V0 is an auditable side-by-side layer. A company/entity is not a ticker: one entity can have many listings, and listings retain their own symbol, venue, MIC, currency, country, provider identity, calendar, and price series. OpenFIGI ticker mapping provides listing/security identity; it does not by itself solve company grouping. V0.2 measures `anchor listing -> ISIN -> LEI -> expanded LEI ISIN set -> sibling listing attach`: one validated anchor ISIN can identify the company LEI, and GLEIF LEI expansion uses paginated `GET /api/v1/lei-records/{LEI}/isins` to conservatively attach sibling listings without requiring every listing to have its own provider ISIN. A second anchor can search GLEIF legalName from OpenFIGI or internal listing names using conservative query variants, but that creates only a candidate LEI; attachment requires exact conservative normalized name, compatible country/address context, one surviving LEI, acceptable GLEIF entity state, and compatible expanded ISINs. The compatible-ISIN gate fails closed: allowed prefixes are derived from explicit listing country or supported provider suffixes, and blank/unknown listing geography is `needs_manual_review` rather than a wildcard. Hong Kong listings use an explicit issuer-prefix policy (`HK`, `CN`, `KY`, `BM`, `GB`) so Cayman/BVI-style issuer ISINs are auditable instead of implicit. Provider ISIN values are validated for format, check digit, and listing context; suspect ISINs are reported and are not used as ISIN anchors. Measurement output now separates unattached tails into `fixable_free`, `requires_provider_or_curated_identity`, and `needs_manual_review`, keeps detailed reject reasons, caps expanded ISIN samples, and emits a `name_anchor_precision_audit` for medium-confidence attachments. Attachments carry provenance/confidence: `isin_direct` and `lei_expansion` are high, `name_anchor_confirmed` is medium, and unresolved/ambiguous cases are `needs_review`. No product read path uses the entity layer yet, no entity tables are published from the measurement command, and no autonomous onboarding or cross-listing price blending is allowed.

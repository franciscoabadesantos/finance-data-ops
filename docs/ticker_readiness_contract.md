...
- `symbol_data_coverage` is a diagnostic operational table. It can be rebuilt from current materialized source tables with `scripts/rebuild_symbol_data_coverage.py` and must not be the sole source of truth for tracked search.
- In rebuilt coverage, `market_data_available=true` means materialized price rows exist in `source_cache.market_price_daily`; quote-only rows do not prove market coverage.

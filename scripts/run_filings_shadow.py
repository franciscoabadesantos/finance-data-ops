#!/usr/bin/env python3
"""Run SEC EDGAR filings in manual shadow mode only."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run SEC EDGAR filings in manual shadow mode only.")
    ...
    parser.add_argument("--refresh", action="store_true", help="Bypass cacheable SEC raw responses.")
    parser.add_argument("--request-sleep-seconds", type=float, default=0.2, help="Pause between live SEC requests.")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.dry_run and settings.database_dsn:
        repository = PostgresSecFilingsShadowRepository(database_dsn=settings.database_dsn)

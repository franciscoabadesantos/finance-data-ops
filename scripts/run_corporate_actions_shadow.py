#!/usr/bin/env python3
"""Run manual corporate-actions shadow ingestion; never writes canonical tables."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run corporate-actions in manual shadow mode only.")
    ...
    parser.add_argument("--request-sleep-seconds", type=float, default=0.25, help="Pause between live provider requests.")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.dry_run and settings.database_dsn:
        repository = PostgresCorporateActionsShadowRepository(database_dsn=settings.database_dsn)

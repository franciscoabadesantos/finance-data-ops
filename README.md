...
OpenFIGI ticker mapping is not sufficient by itself for company/entity grouping. V0 treats ticker-mapping FIGIs as listing/security identity and emits audit rows when company-level identity is missing. V0.2 measures provider ISIN anchors through `ISIN -> LEI -> expanded LEI ISIN set`, using paginated GLEIF `GET /api/v1/lei-records/{LEI}/isins`. A valid ISIN plus GLEIF LEI is strong evidence; if the ISIN country prefix differs from listing country, that mismatch is diagnostic rather than an automatic suspect value, and direct attachment is allowed only when the listing/OpenFIGI/internal name matches the GLEIF legal name under conservative normalization. Those direct attachments report `isin_direct`/high when the prefix is compatible and `isin_direct_prefix_mismatch_name_confirmed`/high when the prefix mismatch is name-confirmed. If direct prefix-mismatch confirmation fails, the listing can still proceed to the separate legalName anchor path. `name_anchor_confirmed` remains fail-closed and medium confidence because it requires exact conservative name normalization, compatible country/address context, one surviving LEI, acceptable GLEIF entity state, and compatible expanded ISINs. Missing geography or no compatible expanded ISIN goes to review or the provider/curated tail instead of accepting name-only grouping. Measurement output classifies unattached listings into `fixable_free`, `requires_provider_or_curated_identity`, and `needs_manual_review`, includes listing-group profiling (`single_listing` vs `multi_listing_candidate`), caps expanded ISIN lists to samples, and includes a `name_anchor_precision_audit` view. GLEIF coverage is partial, and pairs with no valid anchor or confirmed name candidate remain blocked. `--apply-cache` on the measurement command may write raw cache rows only; entity tables remain unpublished until the acceptance set proves the chain.

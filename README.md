...
OpenFIGI ticker mapping is not sufficient by itself for company/entity grouping. V0 treats ticker-mapping FIGIs as listing/security identity and emits audit rows when company-level identity is missing. V0.2 measures validated provider ISIN anchors through `ISIN -> LEI -> expanded LEI ISIN set`, using paginated GLEIF `GET /api/v1/lei-records/{LEI}/isins`. When no ISIN anchor exists, OpenFIGI/listing names may create a GLEIF legalName candidate only; legalName search uses conservative query variants from OpenFIGI and internal names, but the listing attaches with `name_anchor_confirmed` only after exact conservative name normalization, compatible country/address context, one surviving LEI, acceptable GLEIF entity state, and compatible expanded ISINs. The compatible-ISIN gate fails closed: listing country must come from explicit metadata or a supported provider suffix, and missing geography goes to review instead of accepting any expanded ISIN. Hong Kong listings use an explicit accepted issuer-prefix policy (`HK`, `CN`, `KY`, `BM`, `GB`). Measurement output classifies unattached listings into `fixable_free`, `requires_provider_or_curated_identity`, and `needs_manual_review`, with explicit reasons such as OpenFIGI not found, legal-name search ambiguity, valid ISIN without GLEIF LEI, suspect yfinance ISIN, and missing provider ISIN. Normal output caps expanded ISIN lists to samples and includes a `name_anchor_precision_audit` view so medium-confidence attachments can be reviewed before scaling. GLEIF coverage is partial, and pairs with no valid anchor or confirmed name candidate remain blocked. `--apply-cache` on the measurement command may write raw cache rows only; entity tables remain unpublished until the acceptance set proves the chain.

...
def test_write_parquet_table_roundtrip_leaves_no_temp_files(tmp_path) -> None:
    ...
    write_parquet_table("market_price_daily", frame, cache_root=str(tmp_path))

    out = read_parquet_table("market_price_daily", cache_root=str(tmp_path))
    ...
    leftovers = [p.name for p in table_path("market_price_daily", cache_root=str(tmp_path)).parent.iterdir()]
    assert leftovers == ["market_price_daily.parquet"]
